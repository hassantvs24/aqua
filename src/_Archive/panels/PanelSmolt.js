...
const SmoltModel = (props) => {
  ...
  const group = useRef()
  const { nodes, materials, animations } = useLoader(GLTFLoader, '/models/juan/009/Smolt-Fish-alphatex-8.glb')
  ...
  useFrame((state, delta) =>
  {
   ...
   let degrees = getMouseDegrees (props.mouse.current.x, props.mouse.current.y, .1);
  ...
  return (
    <group  position={[-12, -3, 0]} rotation={[0,-106, 0]}  scale={[.33, .33, .33]}>
...
const PanelSmolt = () => {
  ...
  return (
    <>
    <div id="panel-smolt"  style={{padding:'56.25% 0 0 0',position:'relative'}}>
      <video preload='true' id="deep-water-vid" autoPlay={true} muted={true} loop={true} src={deepwWaterVid} ></video>

      <Canvas /* style={{opacity:'.5'}} */ onMouseMove={e => (mouse.current = getMousePos(e))} id="smolt-canvas" shadowMap camera={{ position: [0, 0, 15] }}>
        ...
        */}
        ...
        {/*
      <Canvas onMouseMove={e => (mouse.current = getMousePos(e))} id="fishTest-canvas" shadowMap camera={{ position: [0, 0, 15] }}>
...
/* SMOLT FISH 6
const group = useRef()
  const modelOpac = useRef();
  const { nodes, materials, animations } = useLoader(GLTFLoader, '/models/juan/006/Smolt-Fish-6.gltf')
  const actions = useRef()
  const [mixer] = useState(() => new THREE.AnimationMixer())
  const [opac, setOpac] = useState(.5); 
  ...
  return (
    <group ref={group} {...props} dispose={null}>
    <group  rotation={[0, 45, 0]} position={[-12, -3, 0]} scale={[1, 1, 1]}>
...
/*
const group = useRef()
  const { nodes, materials, animations } = useLoader(GLTFLoader, '/models/juan/006/Smolt-Fish-6-settingfromadult-46.gltf')
 ...
 useEffect(() => {
  ...
  return (
    <group ref={group} {...props} dispose={null}>
    <group  rotation={[0, 45, 0]} position={[-12, -5, 0]} scale={[1, 1, 1]}>
      <group rotation={[0, 0, 0]} scale={[0.3, 0.3, 0.3]}>
          <primitive object={nodes.Atlantic_salmonhead} />
        </group>
        <skinnedMesh
...
/*
  ...
  useEffect(() => {
    actions.current = {
      ...
      materials['Salmon Final Texture'].opacity = 0.5
      materials['Salmon Final Texture'].transparent = true;
