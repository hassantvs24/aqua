...
const SmoltModel = (props) => {
  ...
  useFrame((ani, delta) => {
    ...
    switch (true) {
      ...
      // Smolt Scale into Pen - scroll down
        if (smoltGrowth.current.scale.x > 0 && penPassed === false) {
          ref.current.visible = true
          ...
          smoltGrowth.current.scale.z =
           ...
           // console.log('smoltGrowth.current.scale.x >',smoltGrowth.current.scale.x)
               // Show Pen if scaled all the way
            if (smoltGrowth.current.scale.x <= 0){
            //  console.log('Smolt all the way')
              outPen = true
              smoltZeroScale()
         //     penShrunk = false
          //    penFull = false
        ...
        // Smolt Scale out Pen - scroll up
        if (
          smoltGrowth.current.scale.x < smoltHatchFullScale +0.1
          && penFull === true && penPassed === true
        ) {
        //  ref.current.visible = true
         // console.log('Smolt Scale out Pen - scroll up', ref.current.visible, 'ref.current.visible', penPassed, 'panPassed', penFull, 'penFull')
          ...
          smoltGrowth.current.scale.z =
            smoltGrowth.current.scale.z + smoltPenRate
        /*
            if (smoltGrowth.current.scale.x >= smoltHatchFullScale){
              //  console.log('Smolt all the way')
                outPen = true
              //  penShrunk = false
              } */
      ...
      // Smolt scale into Pen - scroll up
      case curTop >= scrollPenOut && curTop <= scrollGrowAdult:
        ref.current.visible = true

        if (smoltGrowth.current.scale.x > 0 && penPassed === true) {
          //  console.log('Pen In',ref.current.position.y,'PosY', smoltHatchStartPosY, 'smoltHatchStartPosY', curTop,'curTop' )
          smoltGrowth.current.scale.x =
            smoltGrowth.current.scale.x - smoltPenRate
          smoltGrowth.current.scale.y =
        ...
        // Smolt Scale out pen - scroll down
        if (
          smoltGrowth.current.scale.x < smoltHatchFullScale
           && penFull === true && penPassed === false
        ) {
          // console.log('Pen Out',ref.current.position.y,'PosY', smoltHatchStartPosY, 'smoltHatchStartPosY', curTop,'curTop' )
...
const PenModel = (props) => {
  ...
  useFrame((ani, delta) => {
    ...
    switch (true) {
      ...
      case curTop >= scrollPenIn && curTop <= scrollPenState:
        ...
        refWater.current.visible = true
         ...
         // Pen scale up - scroll down
        if (
          ref.current.scale.x < penScaleEnd &&
          penPassed === false && ref.current.scale.z < penScaleEnd -.001 && outPen ===  true
        ) {
          //  console.log('Hatch start',ref.current.position.y,'PosY', smoltHatchStartPosY, 'smoltHatchStartPosY', curTop,'curTop' )
          ref.current.scale.x =
          ...
          /*  
            if (ref.current.scale.x >= penScaleEnd){
              //         console.log(penFull,'penFull')
              penFull = false 
            } */
         ...
         // Pen scale down - scroll up
         
        if (ref.current.scale.x > 0 && penPassed === true && ref.current.scale.x > .001) {
          //  console.log('Pen In',ref.current.position.y,'PosY', smoltHatchStartPosY, 'smoltHatchStartPosY', curTop,'curTop' )
          ref.current.scale.x =
   ...
   //      console.log('ref.current.scale.x',ref.current.scale.x)
          // if Pen shrunk back
          if (ref.current.scale.x <= 0.000001){
           //  console.log(penFull,'penFull',penPassed,'penPassed')
           /* setTimeout(function(){ */ penFull = true /* }, 200); */
          }
        }
      ...
      case curTop >= scrollPenOut && curTop <= scrollGrowAdult:
        ...
        // Pen scale down - scroll down
        if (ref.current.scale.x > 0 && penPassed === false && ref.current.scale.x > .001) {
        //  console.log(outPen,'outPen',' Pen scale down - scroll down', 'scrollPenOut && curTop <= scrollGrowAdult:')
          //  console.log('Pen In',ref.current.position.y,'PosY', smoltHatchStartPosY, 'smoltHatchStartPosY', curTop,'curTop' )
          ref.current.scale.x =
            ref.current.scale.x - penScaleDownRate
          ref.current.scale.y =
  ...
  //          console.log(penFull,'penFull')
         ...
         // Pen scale up - scroll up
         if (
          ref.current.scale.x < penScaleEnd -.001 &&
          penPassed === true && outPen === true
        ) {
         // console.log(outPen,'outPen','  Pen scale up - scroll up', 'scrollPenOut && curTop <= scrollGrowAdult')
          //  console.log('Hatch start',ref.current.position.y,'PosY', smoltHatchStartPosY, 'smoltHatchStartPosY', curTop,'curTop' )
          ref.current.scale.x =
            ref.current.scale.x + penScaleUpRate
          ref.current.scale.y =
...
/*
    ...
    case curTop >= scrollAdult && curTop <= scrollHatchery:
     ...
     //   console.log( '  growthScaleFull curTop >= scrollAdult && curTop <= scrollHatchery',  growthScaleFull )
          growthScaleFull = false      
          ref.current.visible = true
          materials['Salmon Final Texture'].opacity = 1
          break
